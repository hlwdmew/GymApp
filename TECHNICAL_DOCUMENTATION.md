# Техническая документация — Zenith Fitness Hub (качалка.com)

## 1. Обзор системы
Zenith Fitness Hub — это современная ERP-система для управления фитнес-клубом премиум-класса. Платформа объединяет функционал для клиентов (расписание, магазин, персональные планы) и администраторов (аналитика, управление контентом).
...
Система использует **Genkit** для генерации:
1. **Планы тренировок**: `aiWorkoutRecommendations` — генерирует упражнения на основе целей и уровня подготовки.
2. **Планы питания**: `generateNutritionPlan` — составляет 7-дневное меню с учетом калорий и предпочтений.
